...
import { SvgIcon, FileUpload } from '@/components/common';
...
import {uploadFileAPI, parseTableDataAPI} from '@/api'
...
const handleTableData = (data:string) => {
	tableData.value = data;
	console.log("11111",data)

	let obj={
		prompt: "你的身份是一个精通Python，能使用pandas等工具进行数据处理和作图的专家。你将会根据我描述的情景和问题给出相应的数据处理和作图代码。你给出的答案中仅包含代码。我要分析的数据如下："+data + "\n请等待用户提问后再回答",
...
const upFile = async (file: any) => {
	...
	// let res = await parseTableDataAPI(file);
	console.log("res", res)
...
<div class="  myinputs"  @drop="drop" @paste="paste" v-else>
	...
	<FileUpload @tableData="handleTableData" />
